...
static iree_status_t iree_vm_shim_dispatch_v(
  ...
  //
  // NOTE: sizeof(iree_vm_abi_rIiii_t) includes trailing struct padding that
  // is not present before the following variadic segment; size through the
  // final field instead.
  const iree_host_size_t params_size =
      offsetof(iree_vm_abi_rIiii_t, i4) +
      sizeof(((iree_vm_abi_rIiii_t*)NULL)->i4);
  bool args_ok = true;
