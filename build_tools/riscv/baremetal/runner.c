...
// Bare-metal single-op runner using the inline HAL.
// Executes a vmfb compiled with:
//   iree-compile --iree-execution-model=inline-static \
//     --iree-hal-target-device=local \
//     --iree-hal-local-target-device-backends=vmvx-inline
// The module depends only on the hal_inline VM module: no HAL device, no
// executable loader, no threads, no proactor -- suitable for bare metal.
...
// Generated by iree-c-embed-data from the compiled vmfb.
#include "simple_mul_module.h"
...
static iree_status_t run(void) {
  ...
  iree_status_t loader_status = iree_hal_loader_module_create(
      instance, /*flags=*/0, /*loader_count=*/1, &elf_loader, host_allocator,
      &hal_loader_module);
  ...
  IREE_RETURN_IF_ERROR(iree_vm_bytecode_module_create(
      ...
      iree_make_const_byte_span(simple_mul_module_create()->data,
                                simple_mul_module_create()->size),
  ...
  IREE_RETURN_IF_ERROR(
      make_input_buffer_view(device_allocator, kFloat4, 4, &arg0));
  IREE_RETURN_IF_ERROR(
      make_input_buffer_view(device_allocator, kFloat2, 4, &arg1));
